...
const BentoCard = ({ id, title, city, desc, duration, theme, img, size }: any) => (
  ...
  >
    <Image src={img} alt={`${title} — ${city}, India`} fill className="object-cover transition-transform duration-1000 group-hover:scale-110" />
...
const FleetCard = ({ name, type, img, features, onClick }: any) => (
  <motion.div variants={itemVariants} className="glass-card rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 group overflow-hidden h-full">
    <div className="relative h-48 md:h-64 mb-6 md:mb-10 overflow-hidden rounded-2xl">
      <Image src={img} alt={`${name} — ${type} for chauffeured India travel`} fill className="object-cover transition-transform duration-700" />
    </div>
...
export default function HomeView() {
  ...
  const faqs = [
    { q: "Do you provide English-speaking drivers?", a: "Yes, all our chauffeurs are fluent in English and trained in hospitality protocols for international guests." },
    { q: "How do you handle medical emergencies?", a: "Our Medical Sanctuary segment provides 24/7 concierge support, including coordination with hospitals and private nursing." },
    { q: "Can you manage transport for large wedding parties?", a: "Yes. Our wedding logistics wing coordinates multi-vehicle fleets, airport meet-and-greet services, and multi-venue shuttles." },
    { q: "Can I customize my Golden Triangle itinerary?", a: "Absolutely. All our itineraries serve as starting points. We build fully custom, flexible plans based on your preferences and pace." },
    { q: "What class of vehicles are in your fleet?", a: "We offer luxury SUVs, executive sedans, and larger vehicles for groups. Every vehicle is well maintained and GPS-tracked." },
    { q: "Are your tour packages all-inclusive?", a: "We operate with full pricing transparency. Base logistics (vehicle, chauffeur, tolls) are covered, while accommodation and special experiences are tailored to your requirements." },
    { q: "Is it safe to travel across India with your service?", a: "Safety is a priority. We use vetted drivers, real-time GPS tracking, and 24/7 support to keep your journey secure and smooth." },
    { q: "Do you accommodate specialized dietary requirements?", a: "Yes. Our teams can arrange for specific dietary needs — including vegan, Sattvic, or allergen-free meals — while maintaining a quality dining experience." }
  ];
  ...
  const packages = [
    ...
    { id: 10, city: "Jungle & Forts", title: "Wildlife & Wonders", desc: "Golden Triangle + Ranthambore Safari. Amer Fort & City Palace guided heritage walk.", duration: "8 Days", theme: "Wildlife", img: "https://images.unsplash.com/photo-1551316679-9c6ae9dec224?q=80&w=2670&auto=format&fit=crop", size: "md:col-span-1 md:row-span-1" },
    { id: 19, city: "Ganges & Spirits", title: "Spiritual Soul", desc: "Golden Triangle + Varanasi Aarti. Exploring Old Delhi's hidden culinary gems.", duration: "10 Days", theme: "Spiritual", img: "https://images.unsplash.com/photo-1561361513-2d000a50f0dc?q=80&w=2670&auto=format&fit=crop", size: "md:col-span-1 md:row-span-1" },
    { id: 12, city: "Ultra Luxury", title: "Luxury Weekend Protocol", desc: "Helicopter transfers & palatial stays. The absolute gold standard of Golden Triangle travel.", duration: "5 Days", theme: "Elite", img: "https://images.unsplash.com/photo-1599661046289-e31897846e41?q=80&w=2574&auto=format&fit=crop", size: "md:col-span-1 md:row-span-1" },
    { id: 28, city: "Island Sanctuary", title: "Andaman Adventure", desc: "Private island sanctuaries and ultra-luxury marine operations in the heart of the ocean.", duration: "7 Days", theme: "Luxury", img: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=2670&auto=format&fit=crop", size: "md:col-span-1 md:row-span-1" },
    ...
    { id: 29, city: "Mountain Mist", title: "Meghalaya Monsoon", desc: "Living root bridges and the wettest places on earth. Professional guided trekking.", duration: "6 Days", theme: "Nature", img: "https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=2574&auto=format&fit=crop", size: "md:col-span-1 md:row-span-1" },
  ];
  ...
  return (
    <SmoothScroll>
      <main className="min-h-screen relative overflow-hidden bg-white">
        ...
        {/* --- Structured Data --- */}
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify([{ "@context": "https://schema.org", "@type": "TravelAgency", "name": "MyTripMyTravel", "description": "Private, chauffeured luxury tours across India — the Golden Triangle, Rajasthan, Kerala, the Himalayas and beyond.", "url": "https://www.mytripmytravel.com", "areaServed": "India" }, { "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": faqs.map(f => ({ "@type": "Question", "name": f.q, "acceptedAnswer": { "@type": "Answer", "text": f.a } })) }]) }} />
        ...
        <section id="hero-section" className="relative min-h-screen flex flex-col justify-center pt-24 pb-12 md:pt-32 md:pb-20 overflow-hidden z-20">
          <div ref={heroImageRef} className="absolute inset-0">
            <Image src="https://images.unsplash.com/photo-1548013146-72479768bada?q=80&w=2676&auto=format&fit=crop" alt="The Taj Mahal in Agra at sunrise" fill className="object-cover" priority />
          </div>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true }} variants={sectionVariants} className="py-12 md:py-24 glass-card border-none rounded-none z-20 relative">
          <div className="container mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12">
            <motion.div variants={itemVariants}><CountingStat value={15} label="Years of Experience" /></motion.div>
            <motion.div variants={itemVariants}><CountingStat value={20} label="Golden Triangle Routes" /></motion.div>
            <motion.div variants={itemVariants}><CountingStat value={24} label="Hour Concierge Support" /></motion.div>
            <motion.div variants={itemVariants}><CountingStat value={100} label="Custom Itineraries" /></motion.div>
          </div>
        </motion.section>
          ...
          <div className="grid md:grid-cols-2 gap-12 md:gap-24 items-center">
            <div className="order-2 md:order-1 flex justify-center">
               <div className="relative aspect-[4/5] w-full max-w-md rounded-[2rem] overflow-hidden glass-card p-2 shadow-2xl">
                  <Image src="/team-1.jpg" alt="The MyTripMyTravel team" fill className="object-cover rounded-xl transition-all duration-700" />
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={sectionVariants} className="py-40 container mx-auto px-6 z-20 relative">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div className="order-2 lg:order-1">
              ...
              <motion.p variants={itemVariants} className="text-sm md:text-lg font-bold text-dark-slate/60 mb-8 md:mb-12 max-w-xl italic">Silence is the ultimate luxury. Our recovery extensions combine restful, comfortable stays with quiet heritage retreats.</motion.p>
            ...
            <motion.div variants={itemVariants} className="relative h-[400px] md:h-[800px] rounded-[2rem] md:rounded-[4rem] overflow-hidden order-1 lg:order-2 shadow-2xl">
              <Image src="https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?q=80&w=2670&auto=format&fit=crop" alt="A calm, private wellness and recovery suite" fill className="object-cover" />
          ...
          <div className="grid md:grid-cols-3 gap-8 mt-20">
            <motion.div variants={itemVariants} className="glass-card p-8 rounded-3xl border border-royal-blue/10 hover:border-sunset-orange/30 transition-all duration-500 group">
              ...
              <p className="text-dark-slate/60 font-bold italic text-sm mb-6">Master-led sessions of Hatha, Vinyasa, meditation and pranayama in Rishikesh and Himalayan sanctuaries.</p>
            ...
            <motion.div variants={itemVariants} className="glass-card p-8 rounded-3xl border border-royal-blue/10 hover:border-sunset-orange/30 transition-all duration-500 group">
              ...
              <p className="text-dark-slate/60 font-bold italic text-sm mb-6">Physician-led classical Ayurveda at AYUSH-certified centres in Kerala — honest recuperative care, not a cure.</p>
            ...
            <motion.div variants={itemVariants} className="glass-card p-8 rounded-3xl border border-royal-blue/10 hover:border-sunset-orange/30 transition-all duration-500 group">
              ...
              <p className="text-dark-slate/60 font-bold italic text-sm mb-6">Master-therapist bodywork — deep-tissue, marma-point and hot-stone therapy for muscular reset and relaxation.</p>
        ...
        <section id="wedding-section" className="relative h-screen flex items-center justify-center overflow-hidden z-20">
          <div ref={weddingImageRef} className="absolute inset-0">
            <Image src="https://images.unsplash.com/photo-1583939003507-28d8b13c19e5?q=80&w=2670&auto=format&fit=crop" alt="A traditional Indian wedding celebration" fill className="object-cover" />
          ...
          <div className="relative z-10 w-full max-w-6xl px-6 text-center">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={sectionVariants}>
              ...
              <motion.div variants={itemVariants} className="glass-card p-12 md:p-20 rounded-[3rem] border-white/20">
                <p className="text-2xl text-white font-bold mb-16 italic opacity-80 underline decoration-sunset-orange underline-offset-8">We orchestrate your union with full guest-logistics support across the Golden Triangle.</p>
