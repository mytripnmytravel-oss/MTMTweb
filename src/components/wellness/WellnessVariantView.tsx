            ...
            <main className="min-h-screen bg-white text-royal-blue overflow-hidden">
                ...
                <section className="relative h-[60vh] flex items-end overflow-hidden">
                    ...
                    <div className="container mx-auto px-6 relative z-10 pb-16">
                        ...
                        <CharBlurIn text={variant.name.toUpperCase()} className="text-4xl md:text-7xl font-semibold text-white uppercase tracking-tight leading-[0.9] block" />
                    </div>
                </section>
                ...
                <section className="py-24 md:py-32 container mx-auto px-6">
                    <div className="max-w-5xl">
                        ...
                        <p className="text-2xl md:text-4xl font-semibold text-royal-blue leading-snug tracking-tight mb-14">
                            {variant.answer}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-4 mt-12">
                            <div className="glass-card rounded-2xl px-7 py-4 border-royal-blue/5 flex items-center gap-3">
                                ...
                                <span className="font-semibold uppercase text-xs tracking-widest text-royal-blue">{variant.duration}</span>
                            </div>
                            <div className="glass-card rounded-2xl px-7 py-4 border-royal-blue/5 flex items-center gap-3">
                                <UserCheck className="text-sunset-orange" size={18} />
                                <span className="font-semibold uppercase text-xs tracking-widest text-royal-blue">{variant.idealFor}</span>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section className="py-20 bg-royal-blue/5">
                    <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16">
                        <div>
                            <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-8">Outcomes</h4>
                        ...
                        <div>
                            <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-8">The Protocol</h4>
                            <div className="space-y-px">
                                {variant.protocol.map((p, i) => (
                                    <div key={i} className="py-6 border-b border-royal-blue/10">
                                        <div className="text-[10px] font-semibold uppercase tracking-[0.3em] text-sunset-orange mb-2">{p.phase}</div>
                ...
                {variant.faqs.length > 0 && (
                    <section className="py-28 container mx-auto px-6">
                        <div className="text-center mb-16">
                            ...
                            <CharBlurIn text={`${variant.name.toUpperCase()} FAQ`} className="text-3xl md:text-6xl font-semibold text-royal-blue uppercase tracking-tight block leading-none" />
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                            ...
                            {siblings.map((s) => (
                                <Link key={s.slug} href={`/wellness/${programme.slug}/${s.slug}`} className="glass-card rounded-3xl p-7 border-royal-blue/5 group hover:border-sunset-orange/30 transition-all flex items-center justify-between">
                                    <span className="font-semibold uppercase text-sm tracking-tight text-royal-blue group-hover:text-sunset-orange transition-colors">{s.name}</span>
