...
import { ChevronRight, ArrowRight, Sun, CloudRain, Users, Calendar } from "lucide-react";
    ...
    return (
        <main className="min-h-screen bg-paper">
            ...
            <section className="relative flex h-[62vh] min-h-[460px] items-end overflow-hidden">
                ...
                <div className="container-x relative z-10 pb-14 pt-32">
                    ...
                    <div className="mb-4 flex items-center gap-2">
                        ...
                        <p className="eyebrow text-paper/70">{monument.name} &middot; {content.monthLabel}</p>
                    </div>
            ...
            <section className="section pt-0">
                <div className="container-x">
                    <div className="card p-8 sm:p-10">
                        <p className="eyebrow eyebrow-accent">{content.monthLabel} conditions for {monument.name}</p>
                        <div className="mt-8 grid gap-5 md:grid-cols-3">
                            <div className="rounded-2xl border border-line bg-paper-dim/60 p-6">
                                <Sun className="text-clay" size={20} />
                                <h3 className="mt-3 text-[13px] font-semibold uppercase tracking-[0.12em] text-stone">Weather and light</h3>
                                <p className="mt-1.5 text-[14px] leading-relaxed text-muted">{cityMonth.climate.weather}. {cityMonth.climate.light}.</p>
                            </div>
                            <div className="rounded-2xl border border-line bg-paper-dim/60 p-6">
                                <Users className="text-clay" size={20} />
                                <h3 className="mt-3 text-[13px] font-semibold uppercase tracking-[0.12em] text-stone">Crowd</h3>
                                <p className="mt-1.5 text-[14px] leading-relaxed text-muted">{cityMonth.climate.crowd}</p>
                            </div>
                            <div className="rounded-2xl border border-line bg-paper-dim/60 p-6">
                                <CloudRain className="text-clay" size={20} />
            ...
            <section className="border-y border-line bg-paper-dim/60 py-20 sm:py-24">
                <div className="container-x">
                    <p className="eyebrow eyebrow-accent">{monument.name} in {content.monthLabel}, operating notes</p>
                    <div className="mt-10 grid gap-6 md:grid-cols-2">
                        {content.points.map((p, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="card p-8">
                                <h3 className="text-xl font-semibold text-ink">{p.label}</h3>
            ...
            <section className="section">
                <div className="container-x">
                    ...
                    <h2 className="display-3 mt-3 text-ink">{monument.name} in {content.monthLabel}, your questions</h2>
                    <div className="mt-10 grid gap-6 md:grid-cols-2">
            ...
            <LeadBlock
                source={`Monument month: ${monument.name} in ${content.monthLabel}`}
                context={{ "Inquiry Type": "Monument", Monument: monument.name, Destination: dest.name, Month: content.monthLabel }}
                heading={`Plan your ${monument.name} visit in ${content.monthLabel}`}
                pitch={`Private, chauffeured visits to the ${monument.name} with timing and access tuned to ${content.monthLabel} in ${dest.name}. Tell us your dates and party size, and we reply with a tailored plan and a transparent quote.`}
                waMessage={`Hi MyTripMyTravel, I would like to visit the ${monument.name} in ${content.monthLabel}.`}
                faqs={content.faqs}
                breadcrumbs={[
