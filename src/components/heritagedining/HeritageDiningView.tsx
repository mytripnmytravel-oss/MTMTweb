    ...
    return (
        <Shell>
            <section className="pt-60 pb-20 container mx-auto px-6">
                ...
                <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.8em] text-sm mb-6">Heritage Dining</h4>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {category.items.map((it, idx) => (
                        <motion.div key={it.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (idx % 3) * 0.06 }}>
                            <Link href={`/heritage-dining/${category.slug}/${it.slug}`} className="block glass-card rounded-2xl p-9 border-royal-blue/5 group hover:border-sunset-orange/30 transition-all duration-500 h-full">
            ...
            <section className="py-20 bg-royal-blue/5">
                <div className="container mx-auto px-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {siblings.map((s) => (
                            <Link key={s.slug} href={`/heritage-dining/${category.slug}/${s.slug}`} className="px-6 py-3 glass-card rounded-2xl border-royal-blue/10 font-semibold uppercase text-[11px] tracking-widest text-royal-blue hover:bg-sunset-orange hover:text-white transition-all duration-500">{s.name}</Link>
                        ))}
                    </div>
                </div>
            </section>
