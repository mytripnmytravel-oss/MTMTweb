        ...
        <Shell>
            <section className="pt-60 pb-20 container mx-auto px-6">
                <nav aria-label="Breadcrumb" className="flex items-center gap-3 mb-8 text-royal-blue/50 font-black uppercase text-[10px] tracking-[0.3em]">
                    ...
                    <Link href="/heritage-dining" className="hover:text-sunset-orange transition-colors">Heritage Dining</Link>
                    <ChevronRight size={12} />
                    <span className="text-sunset-orange">{category.segment}</span>
                </nav>
                <h4 className="text-sunset-orange font-black uppercase tracking-[0.8em] text-sm mb-6">Heritage Dining</h4>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {category.items.map((it, idx) => (
                        <motion.div key={it.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (idx % 3) * 0.06 }}>
                            <Link href={`/heritage-dining/${category.slug}/${it.slug}`} className="block glass-card rounded-[2.5rem] p-9 border-royal-blue/5 group hover:border-sunset-orange/30 transition-all duration-500 h-full">
        ...
        <Shell>
            <section className="pt-60 pb-16 container mx-auto px-6">
                <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-3 mb-8 text-royal-blue/50 font-black uppercase text-[10px] tracking-[0.3em]">
                    ...
                    <Link href="/heritage-dining" className="hover:text-sunset-orange transition-colors">Heritage Dining</Link>
                    <ChevronRight size={12} />
                    <Link href={`/heritage-dining/${category.slug}`} className="hover:text-sunset-orange transition-colors">{category.segment}</Link>
                    <ChevronRight size={12} />
            ...
            <section className="py-20 bg-royal-blue/5">
                <div className="container mx-auto px-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {siblings.map((s) => (
                            <Link key={s.slug} href={`/heritage-dining/${category.slug}/${s.slug}`} className="px-6 py-3 glass-card rounded-2xl border-royal-blue/10 font-black uppercase text-[11px] tracking-widest text-royal-blue hover:bg-sunset-orange hover:text-white transition-all duration-500">{s.name}</Link>
                        ))}
                    </div>
                </div>
            </section>
            ...
            <section className="py-28 container mx-auto px-6">
                <div className="glass-card p-12 md:p-20 rounded-[4rem] bg-sunset-orange text-white text-center shadow-2xl relative overflow-hidden">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tighter leading-none mb-10 relative z-10">Curate a <span className="text-royal-blue">heritage table</span></h2>
                    <Magnetic>
                        <Link href="/booking" className="inline-block relative z-10 bg-royal-blue text-white py-6 px-12 rounded-[2rem] font-black uppercase tracking-widest text-sm hover:bg-white hover:text-royal-blue transition-all duration-500 shadow-xl">Brief the Dining Wing</Link>
                    </Magnetic>
                </div>
            </section>
        </Shell>
