    ...
    return (
        <Shell>
            <section className="pt-60 pb-20 container mx-auto px-6">
                ...
                <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.8em] text-sm mb-6">Intelligence</h4>
                <CharBlurIn text="FREQUENTLY ASKED" className="text-5xl md:text-8xl font-semibold text-royal-blue uppercase tracking-tight leading-[0.85] block mb-8" />
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {topics.map((t, idx) => (
                        <motion.div key={t.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (idx % 3) * 0.06 }}>
                            <Link href={`/faq/${t.slug}`} className="block glass-card rounded-2xl p-9 border-royal-blue/5 group hover:border-sunset-orange/30 transition-all duration-500 h-full">
                                <div className="text-[10px] font-semibold uppercase tracking-[0.3em] text-sunset-orange mb-3">{t.questions.length} questions</div>
                                <h3 className="text-2xl font-semibold text-royal-blue uppercase tracking-tight mb-4 leading-tight group-hover:text-sunset-orange transition-colors">{t.name}</h3>
    ...
    return (
        <Shell>
            <section className="pt-60 pb-16 container mx-auto px-6">
                ...
                <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-5">FAQ</h4>
                <CharBlurIn text={topic.name.toUpperCase()} className="text-4xl md:text-7xl font-semibold text-royal-blue uppercase tracking-tight leading-[0.9] block mb-6" />
            ...
            <section className="pb-24 container mx-auto px-6">
                        ...
                        <motion.div key={x.slug} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }}>
                            <Link href={`/faq/${topic.slug}/${x.slug}`} className="block glass-card rounded-3xl p-8 border-royal-blue/5 group hover:border-sunset-orange/30 transition-all">
                                <div className="flex items-start gap-4 mb-3">
                                    ...
                                    <h3 className="font-semibold text-royal-blue uppercase tracking-tight text-lg leading-tight group-hover:text-sunset-orange transition-colors">{x.q}</h3>
                                </div>
                ...
                <div className="mt-16">
                    <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-5">Other Topics</h4>
                    <div className="flex flex-wrap gap-3">
                        {others.map((o) => (
                            <Link key={o.slug} href={`/faq/${o.slug}`} className="px-6 py-3 glass-card rounded-2xl border-royal-blue/10 font-semibold uppercase text-[11px] tracking-widest text-royal-blue hover:bg-sunset-orange hover:text-white transition-all duration-500">{o.name}</Link>
                        ))}
                    </div>
                </div>
            </section>
    ...
    return (
        <Shell>
            <section className="pt-60 pb-16 container mx-auto px-6">
                ...
                <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-5">{topic.name}</h4>
                ...
                {atom.links && atom.links.length > 0 && (
                    <div className="flex flex-wrap gap-3">
                        {atom.links.map((l, i) => (
                            <Link key={i} href={l.href} className="px-6 py-4 glass-card rounded-2xl border-royal-blue/10 font-semibold uppercase text-[11px] tracking-widest text-royal-blue hover:bg-sunset-orange hover:text-white transition-all duration-500 flex items-center gap-3">
                        ...
                        {siblings.map((s) => (
                            <Link key={s.slug} href={`/faq/${topic.slug}/${s.slug}`} className="block glass-card rounded-3xl p-8 border-royal-blue/5 group hover:border-sunset-orange/30 transition-all">
                                <div className="flex items-start gap-4 mb-4">
                                    ...
                                    <h3 className="font-semibold text-royal-blue uppercase tracking-tight text-base leading-tight group-hover:text-sunset-orange transition-colors">{s.q}</h3>
                                </div>
