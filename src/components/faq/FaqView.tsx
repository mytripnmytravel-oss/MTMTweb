    ...
    return (
        <Shell>
            <section className="container-x pt-40 pb-16">
                ...
                <h1 className="display-1 mt-4 font-medium text-ink">Frequently asked</h1>
                    ...
                    {topics.map((t, idx) => (
                        <motion.div key={t.slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (idx % 3) * 0.06 }}>
                            <Link href={`/faq/${t.slug}`} className="card group block h-full p-7">
                                <div className="text-[12px] font-medium uppercase tracking-[0.12em] text-stone">{t.questions.length} questions</div>
                                <h3 className="mt-3 text-xl font-medium leading-snug text-ink">{t.name}</h3>
    ...
    return (
        <Shell>
            <section className="container-x pt-40 pb-16">
                ...
                <h1 className="display-1 mt-4 font-medium text-ink">{topic.name}</h1>
                    ...
                    {topic.questions.map((x, i) => (
                        <motion.div key={x.slug} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }}>
                            <Link href={`/faq/${topic.slug}/${x.slug}`} className="card block p-7">
                                <h3 className="text-[17px] font-medium text-ink">{x.q}</h3>
    ...
    return (
        <Shell>
            <section className="container-x pt-40 pb-16">
                ...
                <h1 className="display-1 mt-4 font-medium text-ink">{atom.q}</h1>
                <p className="mt-6 max-w-4xl font-display text-[26px] font-medium leading-snug text-ink sm:text-[32px]">{atom.a}</p>
                    ...
                    <div className="mt-10 grid max-w-6xl gap-6 md:grid-cols-2">
                        {siblings.map((s) => (
                            <Link key={s.slug} href={`/faq/${topic.slug}/${s.slug}`} className="card block p-7">
                                <h3 className="text-[17px] font-medium text-ink">{s.q}</h3>
