            ...
            <main className="min-h-screen bg-white text-royal-blue overflow-hidden">
                ...
                <section className="relative h-[56vh] flex items-end overflow-hidden">
                    ...
                    <div className="container mx-auto px-6 relative z-10 pb-14">
                        ...
                        <CharBlurIn text={`${vehicle.name} IN ${dest.name}`.toUpperCase()} className="text-3xl md:text-7xl font-semibold text-white uppercase tracking-tight leading-[0.9] block" />
                    </div>
                </section>
                    ...
                    <div className="max-w-5xl">
                        ...
                        <div className="flex flex-wrap gap-4 mt-12">
                            <div className="glass-card rounded-2xl px-7 py-4 border-royal-blue/5 flex items-center gap-3">
                                ...
                                <span className="font-semibold uppercase text-xs tracking-widest text-royal-blue">{vehicle.passengers} passengers</span>
                            </div>
                            <div className="glass-card rounded-2xl px-7 py-4 border-royal-blue/5 flex items-center gap-3">
                                <Briefcase className="text-sunset-orange" size={18} />
                                <span className="font-semibold uppercase text-xs tracking-widest text-royal-blue">{vehicle.luggage}</span>
                            </div>
                            <Link href={`/destinations/${dest.slug}`} className="glass-card rounded-2xl px-7 py-4 border-royal-blue/5 flex items-center gap-3 hover:bg-sunset-orange hover:text-white transition-all duration-500 group">
                                ...
                                <span className="font-semibold uppercase text-xs tracking-widest">Explore {dest.name}</span>
                            </Link>
                        </div>
                    </div>
                </section>
                    ...
                    <section className="py-24 bg-royal-blue/5">
                        <div className="container mx-auto px-6">
                            <div className="text-center mb-14">
                                ...
                                <CharBlurIn text={`${vehicle.name} · ${dest.name}`.toUpperCase()} className="text-2xl md:text-5xl font-semibold text-royal-blue uppercase tracking-tight block leading-none" />
                            </div>
                ...
                <section className="py-24 container mx-auto px-6">
                    <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-5">{vehicle.name} in Other Cities</h4>
                    <div className="flex flex-wrap gap-3 mb-14">
                        {otherCities.map((c) => (
                            <Link key={c.slug} href={`/fleet/${vehicle.id}/in/${c.slug}`} className="px-6 py-3 glass-card rounded-2xl border-royal-blue/10 font-semibold uppercase text-[11px] tracking-widest text-royal-blue hover:bg-sunset-orange hover:text-white transition-all duration-500">
                                {c.name}
                            </Link>
                        ))}
                    </div>
                    <h4 className="text-sunset-orange font-semibold uppercase tracking-[0.6em] text-xs mb-5">Other Vehicles in {dest.name}</h4>
                    <div className="flex flex-wrap gap-3">
                        {otherVehicles.map((v) => (
                            <Link key={v.id} href={`/fleet/${v.id}/in/${dest.slug}`} className="px-6 py-3 glass-card rounded-2xl border-royal-blue/10 font-semibold uppercase text-[11px] tracking-widest text-royal-blue hover:bg-sunset-orange hover:text-white transition-all duration-500">
                                {v.name}
                            </Link>
                        ))}
                    </div>
                ...
                <section className="py-24 container mx-auto px-6">
                    <div className="glass-card p-12 md:p-20 rounded-3xl bg-royal-blue text-white text-center shadow-md relative overflow-hidden">
                        ...
                        <h2 className="text-3xl md:text-5xl font-semibold uppercase tracking-tight leading-none mb-10 relative z-10">
